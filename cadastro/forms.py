...
class UsuarioForm(BaseForm):
    preview = ['email', 'username', 'no_completo', 'ic_bolsista', 'is_active']
    class Meta:
        model = Usuario
        fields = ['email', 'username', 'no_completo', 'ic_bolsista', 'is_active', 'is_superuser']
...
class ParticipanteForm(GenericForm):
    ...
    def is_valid(self):
        ...
        if super(ParticipanteForm, self).is_valid():
            ...
            if self.cleaned_data['ic_ativo'] and current_participante:
